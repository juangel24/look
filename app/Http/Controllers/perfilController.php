    ...
    function updateProfile1(Request $request) {
        ...
        foreach ($fo as $k => $c) {
            ...
            foreach ($megst1 as $f => $r) {
                ...
                if ($r->publicacion_id == $c->id and $r->usuario_id == $id) {
                    //dd($r->publicacion_id==$c->id and $r->usuario_id===1);
                    $gg[] = [
                        'foto' => $fo[$k], 'cantidad' => $r['cantidad']
                    ];
        ...
        $yes = mmegusta::where('publicacion_id', '=', 103)->where('usuario_id', '=', $usuarios)->first();
        $fos = [];
        $segi = Seguid::where('usuario_id', '=', $usuarios)->get();
        $yeah=$fo;
        foreach ($fo as $k => $c) {
            $au = false;
            foreach ($segi as $f => $r) {
